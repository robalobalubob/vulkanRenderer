...
namespace vkeng {
    ...
    /**
     * @brief Private constructor for the CommandBuffer.
     */
    ...
    /**
     * @brief Destructor that frees the command buffer back to its pool.
     */
    ...
    /**
     * @brief Factory method to create and allocate a new CommandBuffer.
     */
    Result<std::shared_ptr<CommandBuffer>> CommandBuffer::create(
        VkDevice device, VkCommandPool pool, CommandBufferLevel level) {
        
        if (device == VK_NULL_HANDLE) {
            return Result<std::shared_ptr<CommandBuffer>>(Error("Device handle is null"));
        }
        if (pool == VK_NULL_HANDLE) {
            return Result<std::shared_ptr<CommandBuffer>>(Error("Command pool handle is null"));
        }
        ...
        if (result != VK_SUCCESS) {
            return Result<std::shared_ptr<CommandBuffer>>(Error("Failed to allocate command buffer", result));
        }

        auto cmdBuffer = std::shared_ptr<CommandBuffer>(new CommandBuffer(device, pool, commandBuffer));
    ...
    /**
     * @brief Finishes recording commands into the command buffer.
     */
    ...
    /**
     * @brief Resets the command buffer, allowing it to be re-recorded.
     */
    ...
    /**
     * @brief Transitions to the next subpass in the current render pass.
     */
    ...
    /**
     * @brief Records a command to copy data between buffers.
     */
    ...
    /**
     * @brief Records a command to copy data from a buffer to an image.
     */
    ...
    /**
     * @brief Records a pipeline barrier for synchronization.
     */
    ...
    /**
     * @brief Sets the scissor rectangle dynamically.
     */
    ...
    /**
     * @brief Inserts a single debug label.
     */
    ...
    /**
     * @brief Private constructor for the Queue class.
     */
    ...
    /**
     * @brief Submits a batch of command buffers to the queue.
     */
    Result<void> Queue::submit(
        ...
        for (const auto& cmdBuffer : commandBuffers) {
            if (!cmdBuffer) return Result<void>(Error("Null command buffer in submission"));
            if (cmdBuffer->isRecording()) return Result<void>(Error("Cannot submit command buffer that is still recording"));
    ...
    /**
     * @brief Submits command buffers and waits for completion using a fence.
     */
    ...
    /**
     * @brief Waits until the queue has finished all its pending work.
     */
    ...
    /**
     * @brief Private constructor for the Fence class.
     */
    ...
    /**
     * @brief Destructor that destroys the Vulkan fence object.
     */
    Fence::~Fence() {
    ...
    /**
     * @brief Factory method to create a new Fence.
     */
    Result<std::shared_ptr<Fence>> Fence::create(VkDevice device, bool signaled) {
        ...
        if (result != VK_SUCCESS) {
            return Result<std::shared_ptr<Fence>>(Error("Failed to create fence", result));
        }
    ...
    /**
     * @brief Checks the status of the fence without waiting.
     */
    ...
    /**
     * @brief Private constructor for the Semaphore class.
     */
    ...
    /**
     * @brief Destructor that destroys the Vulkan semaphore object.
     */
    Semaphore::~Semaphore() {
    ...
    /**
     * @brief Factory method to create a new Semaphore.
     */
    Result<std::shared_ptr<Semaphore>> Semaphore::create(VkDevice device) {
        ...
        if (result != VK_SUCCESS) {
            return Result<std::shared_ptr<Semaphore>>(Error("Failed to create semaphore", result));
        }
